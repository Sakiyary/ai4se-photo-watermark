...
class PreviewPanel:
    ...
    def __init__(self, parent: tk.Widget,
                 on_position_change: Optional[Callable[[Tuple[int, int]], None]] = None):
        """
        初始化预览面板
    ...
    def _create_widgets(self):
        ...
        try:
            ...
            # 缩放按钮
            ttk.Button(toolbar, text="放大", command=self._zoom_in, width=8).pack(side=tk.LEFT, padx=2)
            ttk.Button(toolbar, text="缩小", command=self._zoom_out, width=8).pack(side=tk.LEFT, padx=2)
            ttk.Button(toolbar, text="适应", command=self._fit_to_window, width=8).pack(side=tk.LEFT, padx=2)
            ttk.Button(toolbar, text="原始", command=self._actual_size, width=8).pack(side=tk.LEFT, padx=2)
    ...
    def _display_image(self):
        ...
        try:
            ...
            # 显示图像
            self.canvas.create_image(x, y, image=self.canvas_image, anchor=tk.CENTER, tags="image")
