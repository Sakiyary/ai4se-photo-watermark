...
class ImageExporter:
    ...
    def export_image(self, image: Image.Image, output_path: str, 
        ...
        try:
            ...
            export_image.save(output_path, format=format_type.upper(), **save_kwargs)
    ...
    def generate_output_filename(self, original_path: str, naming_config: Dict[str, Any],
        ...
        try:
            ...
            # 获取新扩展名
            new_extension = self.SUPPORTED_FORMATS.get(output_format.lower(), {}).get('extension', '.png')
            ...
            if naming_rule == 'original':
                ...
                # 添加前缀
                prefix = naming_config.get('prefix', 'wm_')
                new_filename = prefix + original_stem + new_extension
            elif naming_rule == 'suffix':
                # 添加后缀
                suffix = naming_config.get('suffix', '_watermarked')
                new_filename = original_stem + suffix + new_extension
            else:
                # 默认添加后缀
                new_filename = original_stem + '_watermarked' + new_extension
