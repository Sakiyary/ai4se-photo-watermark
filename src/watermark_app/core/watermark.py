...
class WatermarkProcessor:
    ...
    def create_text_watermark(self, text: str, font_path: Optional[str] = None, 
        ...
        try:
            ...
            # 考虑阴影的额外空间
            margin = max(abs(shadow_offset[0]), abs(shadow_offset[1])) + stroke_width if shadow else stroke_width
            canvas_width = text_width + margin * 2
            canvas_height = text_height + margin * 2
            
            # 创建透明画布
            watermark_img = Image.new('RGBA', (canvas_width, canvas_height), (0, 0, 0, 0))
